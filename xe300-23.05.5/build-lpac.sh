#!/bin/bash
#
# GL-XE300 (OpenWrt 23.05.5) lpac build script
...
echo "║  Target: OpenWrt 23.05.5 / MIPS 24Kc                     ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
SDK_DIR="${HOME}/xe300-23.05.5-sdk/sdk"
OUTPUT_DIR="${SCRIPT_DIR}/output"

# Build on native Linux filesystem to avoid Windows symlink issues
BUILD_DIR="${HOME}/.local/build/lpac-xe300-23.05.5"
...
SDK_EXTRACTED="openwrt-sdk-${OPENWRT_VERSION}-${OPENWRT_TARGET}-${OPENWRT_SUBTARGET}_gcc-12.3.0_musl.Linux-x86_64"
...
compile() {
    ...
    local toolchain_dir="${SDK_DIR}/staging_dir/toolchain-mips_24kc_gcc-12.3.0_musl"
...
clean_all() {
    ...
    log_info "Note: SDK preserved (~/xe300-23.05.5-sdk/)"
}
