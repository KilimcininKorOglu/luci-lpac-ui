#!/bin/bash
#
# GL-XE300 (OpenWrt 21.02.7) quectel_lpad build script
# Target: MIPS 24Kc (QCA9531 @ 650MHz)
# Architecture: mips-openwrt-linux-musl
#
...
echo "║  Target: OpenWrt 21.02.7 / MIPS 24Kc                     ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
SDK_DIR="${HOME}/xe300-21.02.7-sdk/sdk"
OUTPUT_DIR="${SCRIPT_DIR}/output"
BUILD_DIR="${SCRIPT_DIR}/build"

# SDK information (reference for manual installation)
SDK_URL="https://downloads.openwrt.org/releases/${OPENWRT_VERSION}/targets/${OPENWRT_TARGET}/${OPENWRT_SUBTARGET}/openwrt-sdk-${OPENWRT_VERSION}-${OPENWRT_TARGET}-${OPENWRT_SUBTARGET}_gcc-8.4.0_musl.Linux-x86_64.tar.xz"
SDK_ARCHIVE="openwrt-sdk-${OPENWRT_VERSION}.tar.xz"
SDK_EXTRACTED="openwrt-sdk-${OPENWRT_VERSION}-${OPENWRT_TARGET}-${OPENWRT_SUBTARGET}_gcc-8.4.0_musl.Linux-x86_64"
...
# Toolchain and libcurl setup
setup_toolchain() {
    log_step "Setting up toolchain..."

    export STAGING_DIR="${SDK_DIR}/staging_dir"
    export PATH="${SDK_DIR}/staging_dir/toolchain-mips_24kc_gcc-8.4.0_musl/bin:$PATH"
...
build_libcurl() {
    ...
    if [ -f "${target_dir}/usr/lib/libwolfssl.so" ] || [ -f "${target_dir}/usr/lib/libwolfssl.a" ]; then
        ...
        log_warn "SSL/TLS library not found in SDK, will be compiled..."
    ...
    if [ ! -d "$curl_include" ]; then
        log_warn "libcurl headers not found in SDK, will be compiled..."
...
prepare_source() {
    ...
    # Custom Makefile modifications (if needed)
    cd "$BUILD_DIR"


    # Update VERSION (with BUILD_NUMBER)
    ...
    if [ -f "$VERSION_FILE" ]; then
        ...
        log_info "VERSION updated: 1.0.7-${BUILD_NUMBER}"
    else
...
# Compilation
compile() {
    log_step "Compiling quectel_lpad..."

    cd "$BUILD_DIR"

    # OpenWrt staging directories
    local target_dir="${SDK_DIR}/staging_dir/target-mips_24kc_musl"
    local toolchain_dir="${SDK_DIR}/staging_dir/toolchain-mips_24kc_gcc-8.4.0_musl"
    ...
    # SSL/TLS: detect which SSL library libcurl uses
...
echo ""
echo "✅ quectel-lpad installed!"
echo ""
echo "Usage:"
echo "  quectel_lpad -A <activation_code>     # Add profile"
echo "  quectel_lpad -R <profile_id>          # Delete profile"
echo "  quectel_lpad -D 1 -A <code>           # Debug mode"
echo ""
echo "QMI device: /dev/cdc-wdm0"
echo "More info: quectel_lpad -h"
echo ""
...
print_summary() {
    ...
    echo "     (or manually: scp quectel_lpad root@192.168.8.1:/usr/bin/)"
    echo ""
...
# Cleanup function
clean_all() {
    log_step "Cleanup in progress..."
    rm -rf "$BUILD_DIR"
    rm -rf "$OUTPUT_DIR"
    log_info "Cleanup completed ✓"
    log_info "Note: SDK preserved (~/xe300-21.02.7-sdk/)"
}
