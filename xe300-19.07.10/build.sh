#!/bin/bash
#
# GL-XE300 (OpenWrt 19.07.10) quectel_lpad build script for
...
download_sdk() {
    ...
    log_warn "SDK bulunamadı, otomatik indiriliyor..."
...
build_libcurl() {
    ...
    if [ -f "${target_dir}/usr/lib/libwolfssl.so" ] || [ -f "${target_dir}/usr/lib/libwolfssl.a" ]; then
        ...
        log_warn "SSL/TLS kütüphanesi SDK'da bulunamadı, derlenecek..."
    ...
    if [ ! -d "$curl_include" ]; then
        log_warn "libcurl header'ları SDK'da bulunamadı, derlenecek..."
    ...
    # Final check
    if [ ! -d "$curl_include" ]; then
        log_error "libcurl header'ları hala bulunamadı: $curl_include"
        exit 1
    fi

    log_info "SSL/TLS stack: $ssl_lib + libcurl (using SDK prebuilt)"
...
# Compilation
compile() {
    log_step "quectel_lpad derleniyor..."
...
print_summary() {
    ...
    echo "     (or manual: scp quectel_lpad root@192.168.8.1:/usr/bin/)"
    echo ""
